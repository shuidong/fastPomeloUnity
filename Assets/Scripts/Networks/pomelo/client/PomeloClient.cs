...
namespace Pomelo.DotNetClient
{
    ...
    public class PomeloClient : IDisposable
    {
        ...
        private int _initPort;

        private delegate void AsyncInitClient();
        ...
        private void AsyncInit()
        {
            ...
            try
            {
                IPAddress[] addresses = Dns.GetHostEntry(this._initHost).AddressList;
            ...
            if (ipAddress == null)
            {
                throw new Exception("can not parse host : " + this._initHost);
            }
            */

            this.socket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
            IPEndPoint ie = new IPEndPoint(IPAddress.Parse(this._initHost), this._initPort);
            //ipAddress
        ...
        {
            this._initCallback = callback;//by luo
            this._initHost = host;
            this._initPort = port;

            this._asyncDel = this.AsyncInit;
            IAsyncResult ret = this._asyncDel.BeginInvoke(null, null);
